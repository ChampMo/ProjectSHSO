...
router.get('/api/count_order_sell/', async (req, res) => {
    ...
    try {
        ...
        const count_shop = await db.query('SELECT * FROM Order_list NATURAL JOIN Order_Product NATURAL JOIN Customer NATURAL JOIN product WHERE seller_id = ? AND status_order = ?;', [sseller, 'Wait']);
...
router.post('/api/order_product_sell/', async (req, res) => {
    ...
    try {
        ...
        const product_info_cart = await db.query('SELECT * FROM Order_list NATURAL JOIN Order_Product NATURAL JOIN Customer NATURAL JOIN product NATURAL JOIN Picture_Product WHERE seller_id = ? AND status_order = ? AND product_id = ?;', [seller_id, 'Wait', product_id]);
